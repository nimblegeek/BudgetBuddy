...
page = st.sidebar.radio("Go to",
                        ["Dashboard", "Add Expense", "Budget Planning"])
...
if page == "Dashboard":
    ...
    with col1:
        ...
        category_expenses = monthly_expenses.groupby(
            'category')['amount'].sum()
    ...
    with col2:
        ...
        current_budget = st.session_state.budgets.iloc[
            -1] if not st.session_state.budgets.empty else pd.Series()
        ...
        for cat in categories:
            ...
            actual_values.append(
                float(monthly_expenses[monthly_expenses['category'] == cat]
                      ['amount'].sum()))
    ...
    with st.form("expense_form"):
        ...
        if st.form_submit_button("Add Expense"):
            save_expense(date.strftime('%Y-%m-%d'), category, description,
                         amount)
